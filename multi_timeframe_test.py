...
def detect_trends(ohlc_path, json_path, timeframe):
    """
    Detect trends using pattern recognition and compare with reference JSON
    """
    # Load OHLC data
    ...
    print(f"Pattern-only detection - Found {uptrend_count} uptrends and {downtrend_count} downtrends")
    
    # Get reference uptrend and downtrend data
    ref_uptrends = ref_df[ref_df['type'] == 'uptrendStart']
    ref_downtrends = ref_df[ref_df['type'] == 'downtrendStart']
    ...
    output_file = f"results_{timeframe}_pattern.csv"
    df.to_csv(output_file, index=False)
    print(f"\nResults saved to {output_file}")
    
    return up_match_pct, down_match_pct, overall_match_pct

def main():
    """Run trend detection on multiple timeframes"""
    parser = argparse.ArgumentParser(description='Test trend detection on multiple timeframes')
    parser.add_argument('--timeframes', nargs='+', default=['1d', '4h', '1h'], 
                        help='Timeframes to test (default: 1d 4h 1h)')
    ...
    for tf in args.timeframes:
        ...
        if not os.path.exists(json_path):
            print(f"Warning: {json_path} not found, skipping {tf} timeframe")
            continue
        
        up_pct, down_pct, overall_pct = detect_trends(ohlc_path, json_path, tf)
        results[tf] = {'up': up_pct, 'down': down_pct, 'overall': overall_pct}
    
    # Print summary
    print("\n=== SUMMARY OF RESULTS ===")
    for tf, res in results.items():
        print(f"{tf}: {res['overall']:.1f}% match (Up: {res['up']:.1f}%, Down: {res['down']:.1f}%)")
